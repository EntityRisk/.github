...
Additionally, the reviewer should not formally approve the release PR until it is ready to merge. Any intermediate approval should be communicated by comment only.
...
- [ ] Run the `prep_release` command installed by `erutils[dev]` from the command line to generate a stub release notes file for the new version. Edit with an English-language summary (see [Guidelines](#guidelines)) and commit the file in place.
